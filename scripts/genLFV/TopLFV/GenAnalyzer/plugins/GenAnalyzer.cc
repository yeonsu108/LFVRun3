...
GenAnalyzer::GenAnalyzer(const edm::ParameterSet& iConfig)
  ...
  h_lepLFVdR = fs->make<TH1D>("lepLFVdR","DeltaR between two leptons",12,0.0,6.0);
  h_lepLFVmass = fs->make<TH1D>("lepLFVmass","Inv. mass between two leptons",20,0.0,400);
  ...
  h_topLFVmass = fs->make<TH1D>("topLFVmass","mass of LFV top",30,0.0,300);
  h_topSMpt = fs->make<TH1D>("topSMpt","Pt of SM top",30,0.0,300);
  h_topSMeta = fs->make<TH1D>("topSMeta","Eta of SM top",20,-4.0,4.0);
  h_topSMmass = fs->make<TH1D>("topSMmass","mass of SM top",50,150,200);
...
void GenAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  topSMeta = SMTop.Eta();
  topSMmass = SMTop.M();
  topLFVmassreco = (cfromLFVTop + muonfromLFVTop + taufromLFVTop).M();
  ...
  h_topSMpt->Fill(topSMpt);
  h_topSMeta->Fill(topSMeta);
  h_topSMmass->Fill(topSMmass);
  h_topLFVmassreco->Fill(topLFVmassreco);
