...
namespace JME {
  ...
  std::vector<float> JetParameters::createVector(const std::vector<Binning>& binning) const {
    ...
    for (const auto& bin : binning) {
      ...
      if (it == m_values.cend()) {
        throw std::runtime_error(std::string("JER parametrisation depends on '") + JetParameters::binning_to_string.left.at(bin) +
                           std::string("' but no value for this parameter has been specified. Please call the appropriate 'set' ") +
                           std::string("function of the JME::JetParameters object"));
      }
  ...
  JetResolutionObject::Definition::Definition(const std::string& definition) {
    ...
    if (tokens.size() < 3) {
      throw std::runtime_error(std::string("Definition line needs at least three tokens. Please check file format."));
    }

    size_t n_bins = std::stoul(tokens[0]);

    if (tokens.size() < (n_bins + 2)) {
      throw std::runtime_error(std::string("Invalid file format. Please check."));
    }
  ...
  void JetResolutionObject::Definition::init() {
    ...
    for (const auto& bin : m_bins_name) {
      ...
      if (b == JetParameters::binning_to_string.right.cend()) {
        throw std::runtime_error(std::string("Bin name not supported: '") + bin + std::string("'"));
      }
    ...
    for (const auto& v : m_variables_name) {
      ...
      if (var == JetParameters::binning_to_string.right.cend()) {
        throw std::runtime_error(std::string("Variable name not supported: '") + v + std::string("'"));
      }
  ...
  JetResolutionObject::Record::Record(const std::string& line, const Definition& def) {
    ...
    if (tokens.size() < (def.nBins() * 2 + def.nVariables() * 2 + 1)) {
      throw std::runtime_error(std::string("Invalid record. Please check file format. Record: ") + std::string(line));
    }
  ...
  JetResolutionObject::JetResolutionObject(const std::string& filename) {
    ...
    if (!f.good()) {
      throw std::runtime_error(std::string("Can't read input file '") + std::string(filename + "'"));
    }
