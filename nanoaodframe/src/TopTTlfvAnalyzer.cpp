/*
 * TopTTlfvAnalyzer.cpp
 *
 *  Created on: April 9, 2020
 *      Author: Tae Jeong Kim
 */

#include "TopTTlfvAnalyzer.h"
#include "utility.h"

TopTTlfvAnalyzer::TopTTlfvAnalyzer(TTree *t, std::string outfilename, std::string year, std::string syst, std::string jsonfname, string globaltag, int nthreads)
...
void TopTTlfvAnalyzer::defineCuts()
{
...
/*
template <typename T>
void TopTTlfvAnalyzer::defineVar(std::string varname, T function,  const RDFDetail::ColumnNames_t &columns)
{
	NanoAODAnalyzerrdframe::defineVar(varname, function, columns);
}
*/

void TopTTlfvAnalyzer::defineMoreVars()
{
	// define your higher level variables here
        ...
        defineVar("top_reco_whad", ::top_reconstruction_TTLFV, {"cleanjet4vecs","cleanbjet4vecs","muon4vecs","cleantau4vecs"});
        ...
        defineVar("top_reco_prod", ::top_reco_products_TTLFV, {"cleanjet4vecs","muon4vecs","cleantau4vecs","top_reco_whad"});
...
void TopTTlfvAnalyzer::bookHists()
{
...
//        add1DHist( {"h1npvsgood", "Number of good PVs", 100, 0.0, 100.0}, "PV_npvsGood", "evWeight_pglep", "0");
	...
	add1DHist( {"hncleanjetspass", "Passing jetcuts", 10, 0.0, 10.0}, "ncleanjetspass", "evWeight", "00000");
        ...
        add1DHist( {"hchi2_lfvjmu_dEta", "dEta of lfv jet and muon", 25, -5, 5}, "chi2_lfvjmu_dEta", "evWeight","00000");
        add1DHist( {"hchi2_lfvjmu_dPhi", "dPhi of lfv jet and muon", 20, -4, 4}, "chi2_lfvjmu_dPhi", "evWeight","00000");
        add1DHist( {"hchi2_lfvjmu_dR", "dR of lfv jet and muon", 20, 0, 4.0}, "chi2_lfvjmu_dR", "evWeight","00000");
        add1DHist( {"hchi2_lfvjmu_mass", "Mass of lfv jet and muon", 15, 0, 300}, "chi2_lfvjmu_mass", "evWeight","00000");
        
        add1DHist( {"hchi2_lfvjtau_dEta", "dEta of lfv jet and tau", 25, -5, 5}, "chi2_lfvjtau_dEta", "evWeight","00000");
        add1DHist( {"hchi2_lfvjtau_dPhi", "dPhi of lfv jet and tau", 20, -4, 4}, "chi2_lfvjtau_dPhi", "evWeight","00000");
        add1DHist( {"hchi2_lfvjtau_dR", "dR of lfv jet and tau", 20, 0, 4.0}, "chi2_lfvjtau_dR", "evWeight","00000");
        add1DHist( {"hchi2_lfvjtau_mass", "Mass of lfv jet and tau", 15, 0, 300}, "chi2_lfvjtau_mass", "evWeight","00000"); 
